...
void publishSensorData(float temp, float humidity, int flameDetected, String systemStatus) {
  StaticJsonDocument<256> doc;
  doc["temperature"] = temp;
  doc["humidity"] = humidity;
  doc["flame_detected"] = flameDetected; //0=có lửa, 1= không

  //Xác định trạng thái HT
  systemStatus = "normal";
    bool warningCondition = (temp >=50 && temp<70) || (flameDetected ==0);
    bool dangerCondition = temp >= 70 && (flameDetected ==0);
...
void loop() {
  ...
  if (currentTime - lastPublishTime >= publishInterval) {
    ...
    Serial.printf(
      ...
      temp, humidity, flameDetected == 0 ? "Detected" : "Not detected", systemStatus.c_str()
    );
    Serial.println("------------------------------------");
